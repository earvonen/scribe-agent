...
5. For each issue, calls MCP tool **`SCRIBE_MCP_CREATE_ISSUE_TOOL`** (default `create_issue`) with `labels: ["agent"]` when `assign_to_agent` is true.
...
Defaults assume common GitHub MCP parameters for `create_issue`: `owner`, `repo`, `title`, `body`, and optionally `labels` as a **list of strings**. If your server differs, set **`SCRIBE_MCP_CREATE_ISSUE_EXTRA_JSON`** to merge extra kwargs or override names.
...
| `SCRIBE_MCP_CREATE_ISSUE_TOOL` | No | `create_issue` | MCP tool name for creating issues. |
